...
namespace WebApplication3.repo
{
    public class MenuRepository : IMenuRepository
    {
        ...
        public List<MenuItem> GetMenuItemsByCategory(int category)
        {
            var menuItems = new List<MenuItem>();
            using var connection = new SqlConnection(_connectionString);
            string query = "SELECT * FROM MenuItemTable WHERE course_type = @category";
            using var command = new SqlCommand(query, connection);
        ...
        {
            using var connection = new SqlConnection(_connectionString);
            string query = "SELECT * FROM MenuItemTable WHERE menuItemID = @id";
            using var command = new SqlCommand(query, connection);
        ...
        {
            using var connection = new SqlConnection(_connectionString);
            string query = "UPDATE MenuItemTable SET quantity = quantity + @change WHERE menuItemID = @id";
            using var command = new SqlCommand(query, connection);
